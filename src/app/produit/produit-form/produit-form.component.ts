...
import {
  ...
  Produit,
  Variations,
} from 'src/app/Models/produit.model';
...
export class ProduitFormComponent {
  ...
  editorContent: any = ''; // Contenu de l'éditeur
  ...
  ngOnInit(): void {
    ...
    // Ajoute un délai pour forcer la mise à jour de l'éditeur
    setTimeout(() => {
      this.description_longue = this.produit.description_longue
      this.editorContent = this.produit.description_longue
        ? `${this.produit.description_longue}`
        : '';
  ...
  onEditorChanged(event: any) {
    ...
    const plainText = event.html.replace(/<\/?[^>]+(>|$)/g, '');
    console.log(plainText);
    this.description_longue = plainText;
  }
      ...
      if (Array.isArray(res.message)) {
        ...
        res.message.forEach(
          (variation: {
            ...
            const existingVariation = this.TbVariations.find(
              (v) => v.nom === variation.nom
            );
        ...
        console.error(
          "La structure de res.message n'est pas un tableau",
          res.message
        );
      }
    });
  ...
  saveChangesVariations() {
    const selectedCombinations = this.productCombinations.filter(
      (combination) => combination.selected
    );
    if (selectedCombinations.length === 0) {
      this.globaService.toastShow(
        'Veuillez sélectionner au moins une combinaison avant d’enregistrer.',
        'Information',
        'info'
      );
      return;
  ...
  getCombinations(arrays: any[]): any[] {
    ...
    const combinationsWithFirst = first
      .map((f: any) => combinationsWithoutFirst.map((c) => [f, ...c]))
      .flat();
  ...
  ajouterValeurSupplementaire() {
    const produitIndex = this.TbVariations.findIndex(
      (v) => v.nom === this.variation.nom
    );
    if (produitIndex > -1) {
      // Vérifier si la valeur supplémentaire existe déjà pour cette variation
      const valeurExisteDeja = this.TbVariations[produitIndex].valeurs.some(
        (v) => v.valeur === this.additionalValue
      );
  ...
  onValueChange(
    ...
    console.log(
      `Changement détecté dans ${field} de la variation "${this.TbVariations[variationIndex].nom}"`
    );
  ...
  ajouterVariation() {
    ...
    const index = this.TbVariations.findIndex(
      (v) => v.nom === this.variation.nom
    );
  ...
  addNewValue() {
    if (this.selectedVariationIndex !== null) {
      ...
      if (
        this.newValeur &&
        !selectedVariation.valeurs.some(
          (v) => v.valeur === this.newValeur.trim()
        )
      ) {
