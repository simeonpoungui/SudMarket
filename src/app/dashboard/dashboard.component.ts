...
import {
  GetNotification,
  NotificationSotckproduit,
} from '../Models/notifications.stock.produit.model';
...
import {
  GetNotificationCommande,
  NotificationCommande,
} from '../Models/notifications_commandes';
...
export class DashboardComponent {
  ...
  SelectPointDeVent(event: any) {
    const selectedId = Number(event.target.value);
    console.log(selectedId);

    if (selectedId === 0) {
      if (localStorage.getItem('selectedPoint')) {
        localStorage.removeItem('selectedPoint');
      }
    } else {
      localStorage.setItem('selectedPoint', selectedId.toString());
    }
  ...
  loadPointDeVente() {
    ...
    this.pointDeVenteService.getList(point).subscribe((data) => {
  ...
  loadNotifications(): void {
    ...
    this.notificationService
      .getListNotifications(modelnotif)
      .subscribe((response) => {
        this.notifications = response.message;
        this.unreadCount = this.notifications.filter(
          (notification) => !notification.est_lu
        ).length;
      });
  }
  ...
  getListNotificationsCommandes() {
    ...
    this.notificationService
      .getListNotificationsCommandes(notification)
      .subscribe((res) => {
        ...
        this.unreadCountcommande = this.notificationscommandes.filter(
          (n) => n.statut_notification !== 'lu'
        ).length;
      });
  }
